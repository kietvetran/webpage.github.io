...
=== ===
******************************************************************************/
export const getPagingInfo = ( config ) =>{
  ...
  if ( ! config.total || config.total < config.size ) { return null; }
...
export const getPagingList = ( config ) =>{
  ...
  let list = [], next = 2, interval = [index - next - 1, index + next + 1];
