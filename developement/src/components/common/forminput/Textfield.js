...
const Textfield = ({defaultValue, ...custom}) => {   // eslint-disable-line no-unused-vars
    ...
    if ( formated && source.unit ) {
        formated += ' '+source.unit;
    } else if ( input.value && source.unit ) {
        formated = input.value+' '+source.unit;
    }

    return <InputWrapper {...custom} fieldId={fieldId} type={type} isInvalid={isInvalid}>
        {   source.format || source.unit ? <div className={classNames('input-holder', (source.format ? ('-'+source.format) : ''))}>
