    ...
    **************************************************************************/
    _renderUpdateTabPanelMode : function( callback, delay, fStep, lStep, startup ) {
      ...
      var action = function() {
        ...
        //debug('Top: '+top +' S: '+scrolled+' W:'+wSize);
        return helper._needScroll(null,top,scrolled,wSize) ? 
      ...
      if ( opt.formNode ) {
        ...
        try {
          ...
          if ( error ) {
            ...
            $( sum ).insertBefore( (row.size() ? row : data.panel).children().first() );
            $( sum ).insertBefore( data.nextBtn.first() );
            opt.formNode.SB1formValidation.showSummary( data.panel.find('.'+type) );
          }
        } catch( error ) {}
      }
