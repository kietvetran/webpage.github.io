id := 
sb1_form_textfield

range :=
5

name := 
Lite tekstfelt

approved :=

tag :=
tekst
tekstfelt
input


teaser :=
<i class="sb1_lib_textfield"></i>

detail :=
-d_s 
name := 
Hvordan bruke

description :=
Et lite tekstfelt er beregnet for tekster som ikke er over en linje og som kan valideres. 
Det er mulig å legge til et lite tekstfelt på en linje for seg selv, men også som en del 
av en tekst. (Se eksempel nedenfor). 
<a data-rule="{'id':'component_finanshuset_1'}" class="sb1_link_item" href="#">
Se skjemavalidering og Universell utforming
</a> for hvordan feilmeldinger vises i forbindelse med lite tekstfelt.

-d_e
-d_s
name :=
Demo

iframe :=
{PATH}/index.html
-d_e
-d_s
name :=
HTML-kode

code :=
<input type="text"/>
-d_e

-d_s
name :=
CSS-kode

code :=
input[type="text"]:focus {
  border-color: #008ed2;
}
 
input[type="text"] {
  border: 2px solid #d8d8d8;
  line-height: 36px;
  color: #008ed2;
  font-size: 17px;
  border-radius: 4px;
  padding: 0 10px;
}
 
input[type="text"].large {
  line-height: 56px;
  font-size: 26px;
}
-d_e

directory_depending :=
src/fonts
src/bootstrap

less_import :=
../bootstrap/less/variables.less
../bootstrap/less/mixins.less
../bootstrap/less/normalize.less

less_depending :=
src/less/common/variables.less
src/less/mixins/fontface.less
src/less/common/typography.less
src/less/common/form.less
src/less/common/sb1_form_validation.less

js_depending :=
src/js/jquery/1.8.3.jquery.js
src/js/plugins/sb1_form_validation.js
