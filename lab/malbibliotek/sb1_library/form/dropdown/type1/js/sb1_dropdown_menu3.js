...
;(function($) { $.fn.SB1dropdownMenu = function ( config ) {
  ...
  ****************************************************************************/
  var opt = {
    ...
    'search' : '',
    'overviewCounter' : config.overviewCounter === false ? false : true,
    'ignorIndexSort' : config.ignorIndexSort,
    'searchFieldTopSpace' : config.searchFieldTopSpace,
    'afUpdateSelectedCallback' : config.afUpdateSelectedCallback,
    'multiple' : config.multiple || this.hasClass('multiple-selection'),
    'expander' : config.expander === null ? null : (config.expander || 'vertical'),
  ...
  var helper = {
    ...
    **************************************************************************/
    init : function() {
      if ( opt.expander === 'horizontal' ) { 
        opt.main.addClass('horizontal-expanding');
      }
      if ( opt.multiple ) { 
        opt.main.addClass('multiple-selection'); 
        opt.selectedIndex = {};
      }

      if ( opt.scrollable ) { opt.main.addClass('dd-scrollable'); }

      opt.counter = opt.overviewCounter ? 
        $('<div class="counter"></div>').appendTo(opt.main) : null;
      opt.input   = opt.main.find('input[type="hidden"]').on('reset', helper.reset);

      if ( opt.storage ) { helper.initList( opt.input.val() ); }
      ...
      opt.btn     = opt.main.find('>.dropdown-btn');
      opt.list    = opt.main.find('.dropdown-list');
      opt.option  = opt.main.find('.dropdown-option').each(function(i,d){
        opt.label.push( $(d).text() );
      });
      //opt.input  = opt.main.find('input[type="hidden"]').on('reset', helper.reset);
      opt.list.children().each(function(i,dom){
        $(dom).attr( 'tabindex', '-1');
      });

      if ( opt.field ) { opt.field.attr('aria-owns', opt.list.attr('id') ); }

      var prev = opt.input.prev('.dropdown-btn');
      ...
      opt.btn.on('click', helper._click).on('blur', helper._blur).on('focus', helper._focus);
      opt.list.on('click', helper._click)
        .on('mousemove', helper._mousemove).on('mouseout', helper._mouseout);
      //opt.form.on('submit',helper.-submit);

      $(document).on('keydown', helper._keydown);
      opt.list.delegate( '>li', 'blur', helper._listBlur );
      opt.list.delegate( '>li', 'focus', helper._listFocus );
    ...
    initSearchField : function() {
      var node = $(
        '<div class="dropdown-menu-search-field-wrapper">' +
          '<input type="text" class="input-field dropdown-menu-search-field" value="">' +
        '</div>'
      ).appendTo( opt.main.addClass('included-search-field') ); 

      if ( opt.searchFieldTopSpace ) { 
        opt.main.addClass('top-space-search-field'); 
      }

      opt.field = node.find('.dropdown-menu-search-field');
      opt.field.on('focus', helper._searchFieldFocus)
        .on('blur', helper._searchFieldBlur)
        .on('keyup', helper._searchFieldKeyup);
    },

    initList : function( value ) {
      var matched = {}, i = 0;

      opt.storage = JSON.parse( JSON.stringify(opt.storage) );

      if ( value ) {
        var s = value.split(';'), l = s.length, any = false;
        if ( l ) {
          for ( i=0; i<l; i++ ) { matched[s[i]] = 1; } 

          for ( i=0; i<opt.storage.length; i++ ) {
            var v = opt.storage[i].value || opt.storage[i].id;
            if ( matched[v]) {
              opt.storage[i].mode = 'active';
              any = true;
            }
          }

          if ( any ) {
            opt.storage = opt.storage.sort( function(a,b){
              var x = a.mode === 'active' ? 1 : 0; 
              var y = b.mode === 'active' ? 1 : 0;
      ...
      opt.main.find('.dropdown-list').remove();
      opt.list = $(
        '<ul role="listbox" aria-expanded="false" class="dropdown-list"></ul>'
      ).appendTo( opt.main );
      var id = helper._generateId( opt.list );
      opt.main.find('>.dropdown-btn').attr('aria-controls', id);
      ...
      if ( opt.multiple ) {
        opt.tool = $(
          '<div class="dropdown-multiple-tool">'+
            '<span class="dd-multiple-all-toggler icon-check">'+opt.multipleLabel.all+'</span>'+
          '<div>'
        );
        ...
        if ( opt.toolWrapper && opt.toolWrapper.size() ) {
          ...
          opt.listWrapper = $('<div class="dropdown-list-wrapper"></div>')
            .appendTo( opt.main.addClass('has-multiple-tool') )
            .append( opt.tool ).append( opt.list ); 
        }

        opt.tool.find('.dd-multiple-all-toggler').on('click',function(e) {
          e.preventDefault();
    ...
    **************************************************************************/
    setupAccountData : function( data ) {   
      ...
      data.html = '<div class="account-info">'+
    ...
    appendOption : function( startIndex, reset, previous ) {
      ...
      var box = opt.list || opt.main.find('>.dropdown-list');
      if ( ! box.size() ) { return; }
      ...
      if ( startIndex === -1 && helper.isSelectedIndexEmpty() ) {
        temp.push( 
          '<li class="dropdown-option hide" role="option" data-value="" tabindex="-1">'+
            opt.defaultLabel +
          '</li>'
        ); 
        startIndex = 0;
      ...
      if ( loop ) {
        ...
        for ( i; i<j; i++ ) {
          ...
          var type  = 'dropdown-option' + (test ? ' -selected' : '');
          var dataV = (value ? ' data-value="'+value+'"' : '');
          var dataI = ' data-index="'+index+'"';
          var html  = '<li class="'+type+'" role="option"'+dataV+dataI+' tabindex="-1">'+
            '<div class="dd-item-cnt">'+(data.html ||name)+'</div>'+
          '</li>';
          temp.push( html );
        } 
        ...
        if ( horizontal && temp.length<opt.count ) {
          ...
          dn.children().addClass('dd-hidden');
          var text = dn.html();
          for ( var k=0; k<diff; k++ ) {
            temp.push(
              '<li class="dropdown-empty hidden" aria-hidden="true" tabindex="-1">'+
                 text +//'<div class="dd-item-cnt">&nbsp;</div>'+
              '</li>'
            );
          }
        }
      ...
      var cnt = box.children().not('.option-expander,.hide'), size = cnt.size();
      if ( ! temp.length && !s ) { 
        temp.push('<li class="dropdown-empty" aria-hidden="true">Ingen treff</li>');
      } else if ( (loop > i && opt.expander) || horizontal ) {
        var rest = loop-i; 
        var next = opt.count > rest ? rest : opt.count;

        var menu = horizontal ? 'dropdown-expander' : 'dropdown-option';
        navigation = '<li class="'+menu+' option-expander" aria-hidden="true" tabindex="-1">'+            
          ( 
            horizontal ?
              ( '<div class="dd-navigation dd-previous">Forrige</div>'+
                '<div class="dd-navigation dd-next">Neste</div>')
              :
              '<div class="dd-navigation dd-next">Hent '+next+' neste</div>'            
          ) + 
          '</li>';
        ...
        if ( horizontal ) {
          opt.main.removeClass('no-next no-previous');
          if ( i <= opt.count ) { 
            opt.main.addClass( 'no-previous' );
          }

          if ( i>=loop ) { 
            opt.main.addClass( 'no-next' );
          }
        }
      }
      ...
      if ( ! opt.listWrapper && opt.multiple ) {
        opt.listWrapper = $(
          '<div class="dropdown-list-wrapper">'+        
            ...
            '<div>'+
          '</div>'
        ).appendTo( opt.main.addClass('has-multiple-tool') ).append( box ); 
        opt.listWrapper.find('.dd-multiple-all-toggler').on('click',function(e) {
          e.preventDefault();
          helper.toggleAllMultipleSection();
          opt.btn.focus();
          return false;
        });
      } else { box.removeClass('has-multiple-tool'); }
      */
      ...
      if ( horizontal && size ) {
        ...
        var a = ['class', 'data-index'], timer = setInterval( function() {
          ...
          if ( temp[j] ) {
            ...
            if ( n.size() ) {
              var f = [
                n.children().eq(0).addClass('on-close'),
                t.children().eq(0).addClass('new-insert')
              ];
              ...
              setTimeout( function() {
                ...
                setTimeout( function() {
                  f[0].remove(); 
                  f[1].attr('style','width:100%;').removeClass('animation run new-insert before after');
    ...
    reset : function () {
      opt.main.find('.dropdown-option').eq(0).click();
    },
    ...
    showList : function() { 
      ...
      clearTimeout( opt.timer.showListFocus || 0 );
      opt.timer.showListFocus = setTimeout( function(){
        if ( opt.field && opt.field.size() ) {
          if ( ! opt.field.is(':focus') ) {
            opt.field.focus();
          }
        }
        else if ( ! opt.btn.is(':focus') ) {
          opt.btn.focus();
        }
      }, 50 );
    },
    ...
    hideList : function( force ) { 
      if ( opt.ignorHideList && force !== true ) { return; }
      if ( opt.multiple || force !== true ) {
        var delay = opt.multiple ? 100 : 50;
        clearTimeout( opt.timer.hidelist || 0 );
        opt.timer.hidelist = setTimeout( helper._hideOptionList, delay );
      } else { helper._hideOptionList(); }
    },

    updateSelected : function( item, multipleForce, previous ) {
      ...
      if ( opt.multiple && opt.cloned ) {
        ...
        if ( list.length >= loop ) { wrapper.addClass('-all--selected');    } 
        else                       { wrapper.removeClass('-all--selected'); }
      }
      ...
      if ( signleHandler ) {
        ...
        if ( child.eq(0).hasClass('dd-item-cnt') ) { 
        ...
        if ( child.size() ) {
          ...
          text = '<span class="text">'+out.join('</span> <span class="text other">')+'</span>';
        }

        setTimeout( function() {
          helper._verifyTab( previous, slc );
        }, 50 );
      }

      if ( opt.btn ) { 
        var labelholder = opt.btn.find('.label-holder');
    ...
    setSelected : function( item ) {
      ...
      if ( opt.multiple ) {
        ...
        helper.updateSelected( item.eq(0).addClass(mode), null, slc );
        helper.scrollBack();
      }
    ...
    changeOptionFocus : function( up, force ) {
      var index = null, slc = '.hide,.dropdown-empty' +
        (opt.expander === 'horizontal' ? ',.option-expander' : '');
      ...
      if ( typeof(up)==='boolean' && next.hasClass('option-expander') ) {
        helper._runExpanding(next, true);
        return helper.changeOptionFocus( null, index );
      }

      if ( typeof(up) === 'boolean') { helper._changeAriaHidden( next ); }
      //helper._changeAriaHidden( next );
      return next;
    },
    ...
    toggleAllMultipleSection : function( force ) {
      ...
      var mode = '-selected', what = '-all--selected'; 
      ...
      if ( force ) {
        ...
        opt.list.find('.dropdown-option').addClass( mode );
      } 
      else {
        opt.selectedIndex = {};
        opt.list.find('.dropdown-option').removeClass( mode );
      }

      helper.updateSelected( null, force );
    },

    verifyChangeSelectText : function( index ) {
      var field = opt.field;
      if ( ! field || ! field.is(':focus') || isNaN(index) ) { return; }

      var list = this.matched || this.limited || this.cloned || [];
      if ( ! list[index] ) { return; }

      var name = list[index].text || list[index].label || '';
      if ( ! name ) { return; }

      var text = opt.keyupSearchText || '';
      helper._selectText(text.length,name.length,field.val(name));
    },

    /*************************************************************************
    === INTERNAL FUNCTIOn ===
    **************************************************************************/
    _hideOptionList : function() {
      opt.clickedExpanded = null;
      opt.main.removeClass('-open -on_focus'); 
    ...
    _changeAriaHidden: function( node ) {
      opt.ignorHideList = true; 
      node.focus();
      setTimeout( function() { 
        opt.ignorHideList = null; 
      }, opt.multiple || opt.field ? 220 : 100 );

      if ( opt.field ) { opt.field.attr('tabindex','-1'); }

      /*
      opt.btn.find('>.shy-info').remove();
      var holder = opt.btn.find('>.info-holder');
      if ( ! holder.size() && node && node.size() ) { 
        holder = opt.btn.html( 
          '<div aria-hidden="true" class="info-holder">'+opt.btn.html()+'</div>' 
        );
      } 
      else if ( holder.size() && (! node || ! node.size()) ) {
        opt.btn.html( holder.html() );
      }

      if ( node && node.size() ) {
        var a = node.find('a');
        opt.btn.append(
          '<div class="shy-info" style="visibility:hidden;position:absolute;left:-100;top:0">'+
            (a.size() ? a.hmlt() : node.html() )+
          '</div>'
        ).focus();
      }
      */
    ...
    _displayCountView : function( sum ) {
      ...
      var view = opt.list.children().not('.hide,.option-expander,.dropdown-empty').size();
    ...
    _verifyTab : function( slc, item ) {
      ...
      if ( rule[tab] ) {
        wrapper = $( rule[tab] );
        if ( wrapper.size() ) {
          wrapper.addClass('-hide').find('input').removeAttr('required');
        }
      }
      ...
      if ( rule[tab] ) {
        wrapper = $( rule[tab] );
        if ( wrapper.size() ) {
          wrapper.removeClass('-hide').find('input').attr('required','true');
        }
      }
    },
    ...
    _listFocus : function( e ) {
      clearTimeout( opt.timer.listBlur || 0 );
    },

    _listBlur : function( e ) {
      clearTimeout( opt.timer.listBlur || 0 );
      opt.timer.listBlur = setTimeout( function() {
        if ( ! helper.isListOpen() || opt.btn.is(':focus') ) { return; }
        if ( opt.field && opt.field.is(':focus') ) { return; }
        helper.hideList();
      }, 50 );
    },
    ...
    _click : function( e ) {
      ...
      var exp = 'option-expander', btn = 'dropdown-btn';
      e.preventDefault();

      if ( node.hasClass('dd-multiple-all-toggler') ) {
        helper.toggleAllMultipleSection();
      }
      else if ( node.hasClass(exp) || parent.hasClass(exp) ) {
        if ( opt.expander==='horizontal') {
          if ( node.hasClass('dd-navigation') ) { 
        ...
        helper.setSelected( node.closest('.dropdown-option') );
        if ( opt.multiple ) { 
          clearTimeout( opt.timer.hideList || 0 );
    ...
    _keydown : function( e, t, c ) {
      //clearTimeout( opt.timer.docKeydown || 0 )
      //opt.timer.docKeydown  = setTimeout( function() {
        var code = c || e.keyCode, open = helper.isListOpen();
        if ( ! opt.btn.is(':focus') && (! opt.field || ! opt.field.is(':focus')) ) {
          var focus = opt.list.children().filter('.focus');
          if ( ! focus.size() || ! open ) { return; }
        }
        //var node = t || $(e.target), id = node.attr('id') || '-', code = c || e.keyCode;
        if ( code === 9 )  { 
          helper.setSelected( opt.list.children().filter('.focus') );
          if ( open ){ helper.hideList( true ); }
          return;
        }
        ...
        //else if ( ! opt.field || ! opt.field.is(':focus') ) {
        else {
          ...
          opt.timer.erase = setTimeout( function() { 
            opt.search = ''; 
            //if ( opt.field ) { 
            //  opt.field.removeAttr('tabindex').val( opt.search );
            //  opt.main.removeClass('inactive-search-field');
            //}
          }, 2000 );
          setTimeout( function() { 
            if ( opt.field && ! opt.field.is(':focus') ) { 
              opt.field.removeAttr('tabindex').val( opt.search );
              opt.main.removeClass('inactive-search-field');
              if ( ! opt.main.hasClass('focus-on-search-field') ) {
                clearTimeout( opt.timer.erase || 0 );
                opt.field.focus();
              }
              return;
    ...
    _searchFieldFocus : function( e ) {
      ...
      var mode = 'focus-on-search-field', has = opt.main.hasClass( mode );
      ...
      opt.main.addClass( 'focus-on-search-field');
    },

    _searchFieldBlur : function( e ) {
      helper._blur( null, opt.btn.removeAttr('tabindex') );
      opt.main.removeClass( 'focus-on-search-field');
    },
    ...
    _searchFieldKeyup : function( e ) {
      ...
      if ( code===13 || code===32 ) {
        ...
        if ( code === 13 ) { opt.main.addClass('inactive-search-field'); }
        return;
      }

      clearTimeout( opt.timer.search || 0 );
      opt.timer.search = setTimeout( function() {
        var field = opt.field, text = field.val() || '', newText = '';
        if ( text.toLowerCase() == opt.keyupSearchText ){ return; }

        opt.keyupSearchText = text.toLowerCase();
        opt.matched         = [];

        var reg   = helper._createRegExp(text,true,true,true);
        var count = opt.count, loop = opt.cloned.length;
        for ( var i=0; i<loop; i++ ) {
          var name = opt.cloned[i].text || opt.cloned[i].label || opt.cloned[i].name || '';
        ...
        if ( ! text || code === 8 ) { return; }

        opt.list.children().removeClass('-focus').eq(0).addClass('-focus');
        if ( newText ) {
          helper._selectText(text.length,newText.length,field.val(newText));
        }
      }, 50 );
    ...
    ____searchFieldKeyup : function( e ) {
      var that = this, code = e.keyCode;
      if ( ! that._isCurrent(e) ) { return; }

      if ( (code>=37 && code<=40) || code === 9 ) { return; }
      if ( code===13 || code===32 ) {
        that._keydown( e, that.$btn, code );
        if ( code === 13 ) {
          that.$main.addClass('-inactive-search-field');
        }
        return;
      }

      clearTimeout( that.timer.search || 0 );
      that.timer.search = setTimeout( function() {
        var field = that.$field, text = field.val() || '', newText = '';
        if ( text.toLowerCase() == that.keyupSearchText ){ return; }

        that.keyupSearchText = text.toLowerCase();
        that.matched         = [];

        var reg   = that._createRegExp(text,true,true,true);
        var count = that.viewCount, loop = that.cloned.length;
        for ( var i=0; i<loop; i++ ) {
          var name = that.cloned[i].text || that.cloned[i].label;
          if ( ! name.match(reg) ) { continue; }
          if ( ! newText ) { newText = name; }

          that.matched.push( that.cloned[i] );
          if ( that.matched.length === ++count ) { i = loop+1; }
        }
        that.appendOption( null, true );
        that.$main.removeClass('-inactive-search-field');
        if ( ! that.isListOpen() ) { that.showList(); }

        if ( ! text || code === 8 ) { return; }

        that.$list.children().removeClass('-focus').eq(0).addClass('-focus');
        if ( newText ) {
          that._selectText(text.length,newText.length,field.val(newText));
        }
      }, 50 );
    },
    ...
    _focus: function( e, target ) {
      if ( opt.field && ! target ) {
        return opt.onSelected ? null : opt.field.removeAttr('tabindex').focus();
      }

      clearTimeout( opt.timer.focus || 0 );
      opt.timer.focus = setTimeout( function() {
        var node = target || $(e.target), id = node.attr('id') || '-';
        if ( opt.timer[id] ) { clearTimeout( opt.timer[id] ); }
    ...
    _blur : function( e, target ) {
      ...
      opt.timer[id] = setTimeout( function() { 
        ...
        //helper._changeAriaHidden();
      }, 100 );      
    },

    _mousemove : function( e ) {
      opt.list.children().removeClass('focus');
      $(e.target).closest('.dropdown-option').addClass('focus');
    },
    ...
    _runExpanding : function( expander, keyboard ) {
      ...
      if ( opt.expander === 'horizontal' ) {
        ...
        var next  = expander.hasClass('dd-previous') ? (index-(opt.count*2)) : index;
        if ( next < 0 || next >= loop ) { return; }
        opt.nextIndex = next;
        helper.appendOption( null, null, expander.hasClass('dd-previous'));        
      }
      else {
        expander.remove();
    ...
    _getRule : function ( input ) {
      ...
      if ( typeof(rule.day)   !== 'undefined' ) { node.addClass('-rule-day');  }
      if ( typeof(rule.month) !== 'undefined' ) { node.addClass('-rule-month');}
      if ( typeof(rule.year)  !== 'undefined' ) { node.addClass('-rule-year'); }
    ...
    _generateId : function( node ) {
      var id = node.attr('id');
      if ( ! id ) {
        id = 'auto-'+(new Date()).getTime()+'-'+Math.floor((Math.random()*1000)+1);
        node.attr('id',id);
      }
      return id;
    },
  ...
  //for ( var k in helper ) {
  //  if ( ! k.match(/^(init|setup|-)/i) ) this[k] = helper[k];
  //}
  var method = {};
  for ( var k in helper ) {
