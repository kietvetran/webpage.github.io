  ...
  var helper = {
    ...
    **************************************************************************/
    init : function() {
      ...
      opt.counter = opt.overviewCounter ? 
        $('<div class="sb1_counter"></div>').appendTo(opt.main) : null;
    ...
    initSearchField : function() {
      var node = $(
        '<div class="sb1_dropdown_menu_search_field_wrapper">' +
          '<input type="text" class="sb1_dropdown_menu_search_field" value="">' +
        '</div>'
      ).appendTo( opt.main.addClass('included_search_field') ); 

      if ( opt.searchFieldTopSpace ) { 
        opt.main.addClass('top_space_search_field'); 
      }
    ...
    **************************************************************************/
    ...
    appendOption : function( startIndex, reset, previous ) {
      ...
      if ( startIndex === -1 && helper.isSelectedIndexEmpty() ) {
        temp.push( 
          '<li class="sb1_dropdown_option hide" role="option" data-value="" tabindex="-1">'+
            opt.defaultLabel +
          '</li>'
        ); 
        startIndex = 0;
      ...
      if ( loop ) {
        ...
        if ( horizontal && temp.length<opt.count ) {
          ...
          for ( var k=0; k<diff; k++ ) {
            temp.push(
              '<li class="sb1_dropdown_empty hidden" aria-hidden="true" tabindex="-1">'+
      ...
      if ( ! temp.length && !s ) { 
        ...
        navigation = '<li class="'+menu+' option_expander" aria-hidden="true" tabindex="-1">'+            
          ( 
            horizontal ?
    ...
    hideList : function( force ) { 
      ...
      if ( opt.multiple ) {
        clearTimeout( opt.timer.hidelist || 0 );
        opt.timer.hidelist = setTimeout( helper._hideOptionList, 100 );
    ...
    **************************************************************************/
    ...
    _changeAriaHidden: function( node ) {
      ...
      /*
      opt.btn.find('>.sb1_shy_info').remove();
      var holder = opt.btn.find('>.sb1_info_holder');
      if ( ! holder.size() && node && node.size() ) { 
        holder = opt.btn.html( 
          '<div aria-hidden="true" class="sb1_info_holder">'+opt.btn.html()+'</div>' 
        );
      } 
      ...
      if ( node && node.size() ) {
        var a = node.find('a');
        opt.btn.append(
          '<div class="sb1_shy_info" style="visibility:hidden;position:absolute;left:-100;top:0">'+
