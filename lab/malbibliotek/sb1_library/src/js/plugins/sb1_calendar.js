...
;(function($) { $.fn.SB1calendar = function ( config ) {
  ...
  ****************************************************************************/
  var opt = {
    ...
    'value' : typeof(config.value)==='object' ? config.value : 
        (typeof(config.value)==='number' ? (new Date(config.value)) : null),
  ...
  var helper = {
    ...
    **************************************************************************/
    ...
    setupShortcut : function( shortcut, where, selectedIndex ) {
      ...
      if ( node.size() ) {
        ...
        if ( typeof(opt.afClickOnShortcutCallback)==='function' ) {
          opt.afClickOnShortcutCallback({
            'current': node, 'interval': interval, 'main': opt.main
          });
        }
      }
    ...
    setupController : function() {
      ...
      if ( opt.field ) {
        var render = false, tmp = null;
        opt.field.each( function(i,dom) {
          if ( ! dom.value ) { 
            if ( opt.value ) {
              dom.value = helper._convertDateToText( opt.value );            
            } else { return; }             
          }
          var date = tmp || helper._convertTextToDate( dom.value );
          if ( ! date ) { 
            dom.value = '';
          } else {
    ...
    */
    ...
    _keydownBody : function( e ) {
      if ( ! helper.isCalendarOpen() || ! opt.focusTarget ) { return; }
      var code = e.keyCode, field = opt.field.eq(0);
      var focus = field.is(':focus') || 
        (opt.field.size() > 1 ? opt.field.eq(1).is(':focus') : false);
      if ( ! focus && (code>=38 && code<=40) || code===32 ) { e.preventDefault(); }
    },
    ...
    _resize : function( e ) {
      if ( helper.isCalendarOpen() ) { 
        helper._revealWidget(); 
      } else if ( opt.controller ) {
        clearTimeout( opt.timer.resize || 0 );
        opt.timer.resize = setTimeout( function() {
          var remove = true;
          opt.controller.each( function(i,dom){
            if ( remove && $('#'+dom.id).size() ) { remove = false; }
          });
          if ( remove ) {
            $(window).off( 'resize', helper._resize );
            opt.widget.remove();
          }
        }, 100 );
      } 
    ...
    _navigateFocus : function( code ) {
      ...
      if ( code===38 || code === 40 ) {
        if ( code===38 && number===0 ) { 
          return opt.focusTarget ? opt.focusTarget.focus() : null; 
        }
    ...
    _clickOnShortCut : function( e ) {
      ...
      if ( typeof(opt.bfClickOnShortcutCallback)==='function' ) {
        var answer = opt.bfClickOnShortcutCallback({
          'event': e, 'current': current, 'interval': interval, 'main':opt.main
        });
        if (answer===false ) { return; }
      }
    ...
    _click : function ( e ) {
      ...
      if ( current.hasClass(arrow) ) { 
       helper._clickOnNavigator( current, date );
        ...
        if ( typeof(opt.bfClickOnDayCallback)=='function') {
          var answer = opt.bfClickOnDayCallback({
            'e': e, 'current': current, 'date': date
          });
          if (answer===false ) { return; }          
        }
        helper._clickOnDate( current, date );
        if ( typeof(opt.afClickOnDayCallback)=='function') {
          opt.afClickOnDayCallback({
            'e': e, 'current': current, 'date': date
          });
        }
      }
    ...
    _getCalendarTable : function( date ) {
      ...
      for ( i=0; i<6; i++ ) {
        ...
        for ( j=0; j<7; j++ ) {
          ...
          var b = active && active[t] ? 'true' : 'false';
          column.push(
            '<td role="presentation">'+
              '<a href="#" data-rule="'+r+'" class="'+s+'" role="button"'+(isD ? ' tabindex="-1"':'')+' aria-selected="'+b+'">'+
