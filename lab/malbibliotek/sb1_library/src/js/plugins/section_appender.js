...
;(function($) { $.fn.SB1sectionAppender = function ( config ) {
  ...
  ****************************************************************************/
  ...
  var helper = {
    ...
    **************************************************************************/
    init : function() {
      if ( ! opt.current || ! opt.current.size() ) {
          return;
      }
      ...
      if ( ! opt.tag ) {
          opt.tag = item.eq(0).prop('tagName');
      }
    
      if ( opt.tag.toLowerCase() === 'a' ) {
          opt.tag = 'div';
      }

      var type = 'sb1_section_appender' + (opt.class ? ' '+opt.class : ''); 
