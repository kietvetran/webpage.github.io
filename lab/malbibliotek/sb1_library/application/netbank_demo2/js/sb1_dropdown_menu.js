    ...
    **************************************************************************/
    ...
    appendOption : function( startIndex, reset ) {
      ...
      if ( loop ) {
        ...
        i = typeof(startIndex)==='number' ? startIndex : (opt.nextIndex || 0); 
        debug('I: '+i);
      ...
      var cnt = box.children().not('.option_expander'), size = cnt.size();
      if ( ! temp.length && !s ) 
        temp.push('<li class="sb1_dropdown_empty" aria-hidden="true">Ingen treff</li>');
      ...
      //var cnt = (box.html() || '').replace( /[\s\n\t\r]+/g, '' );
      //if ( opt.expander==='horizontal' && cnt ) {
      if ( opt.expander==='horizontal' && size ) {
        var d = helper.getDuration( cnt.eq(0).children().eq(0) ) || 100;
        var a = ['class', 'data-index'], j = 0, timer = setInterval( function() {
          var n = cnt.eq( j );
          if ( temp[j] ) {
            var t = $( temp[j] );
            if ( n.size() ) {
              ...
              if ( false ) {
                ...
                f[1].addClass('after').insertAfter( f[0].addClass('according_to_after') );
              }
              ...
              setTimeout( function() {
                ...
                setTimeout( function() {
                  f[0].remove(); 
                  f[1].attr('style','width:100%;').removeClass('animation run new_insert');
                  for ( var i=0; i<a.length; i++ ) n.attr(a[i],t.attr(a[i]));
                  f[1].removeAttr('style');
                }, d+100 );
              }, 20 );
        ...
        opt.nextIndex += temp.length; 
      } else {
        opt.nextIndex = size ? size-1 : 0; 
        $( temp.join("\n") ).appendTo( box ); 
      }
