...
;(function($) { $.fn.SB1rememberCard = function ( config ) {
  ...
  ****************************************************************************/
  var opt = {
    'main' : this,
    'continue_callback'      : config.continue_callback,
    'discard_callback'       : config.discard_callback,
    'before_discard_callback': config.before_discard_callback
  };
  ...
  var helper = {
    ...
    **************************************************************************/
    ...
    continueRememberCard : function( e ) {
      if ( typeof(opt.continue_callback) != 'function' ) return;
      opt.continue_callback( opt.main );
    },

    discardRememberCard : function( e ) {
      var duration = helper.getDuration( opt.main ), render = function() {
        if ( typeof(opt.discard_callback)=='function' )
          opt.discard_callback( opt.main, duration );
        opt.main.remove();      
      };
      if ( typeof(opt.before_discard_callback)=='function' )
        opt.before_discard_callback( opt.main, duration );
      opt.main.addClass('discard');
      duration ? setTimeout( render, duration ) : render();
    },
