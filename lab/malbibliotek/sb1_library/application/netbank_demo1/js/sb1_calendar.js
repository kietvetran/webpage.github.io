...
;(function($) { $.fn.SB1calendar = function ( config ) {
  ...
  ****************************************************************************/
  var opt = {
    ...
    'display_table' : typeof(config.display_table)=='number' ? config.display_table : 1
  };
  ...
  var helper = {
    ...
    **************************************************************************/
    ...
    setupController : function() {
      ...
      var isField    = controller.is('input[type="text"]');
    ...
    setupStartCalender : function() {
      var view = opt.view || new Date(opt.now_year,opt.now_month,1,0,0,0,0,0);
      var cnt  = [], cloned = new Date(view.getTime());
      var loop = opt.display_table, i = 0;
      for ( i=0; i<loop; i++ ) {          
        view.setMonth( view.getMonth()+(i? 1: 0));
        if ( view.getTime() > opt.max_time )
          i = loop;
        else {
          cnt.push( 
      ...
      for ( i=cnt.length; i<loop; i++ ) {
        ...
        if ( cloned.getTime() < opt.min_time )
          i = loop;
        else {
          cnt.push( 
            '<div class="sb1_calendar_table_wrapper">'+
    ...
    setupNowDate : function() {
      opt.now_year  = opt.now.getFullYear();
      opt.now_month = opt.now.getMonth();
      opt.now_date  = opt.now.getDate();
      opt.now = new Date( opt.now_year,opt.now_month, opt.now_date,0,0,0,0,0);
      opt.now_time = opt.now.getTime();
    },

    setupMaxMinDate : function() {
      if ( typeof(opt.max)=='number' ) {
        opt.max = new Date(opt.max);
        opt.max_time = opt.max.getTime();
        if ( opt.max_time < opt.now_time ) {
          opt.max = new Date( opt.now_time+opt.a_Day );
          opt.max_time = opt.max.getTime();
        }
      }
      else {
        opt.max = new Date(opt.now_year+100,11,31,0,0,0,0,0);
        opt.max_time = opt.max.getTime();
      }

      if ( typeof(opt.min)=='number' ) {
        opt.min = new Date(opt.min);
        opt.min_time = opt.min.getTime();
        if ( opt.min_time > opt.now_time ) {
          opt.min = new Date( opt.now_time );
          opt.min_time = opt.min.getTime();
        }
      }
      else {
        opt.min = new Date(opt.now_year-100,11,31,0,0,0,0,0);
        opt.min_time = opt.min.getTime();
      }
    },
    ...
    **************************************************************************/
    ...
    _navigateFocus : function( code ) {
      ...
      var out = 'out_of_month', off = 'sb1_disabled', end = 'end_column';
      ...
      if ( code==38 || code == 40 ) {
        ...
        */
        ...
        var next = index + addition, force = false;
    ...
    _forceFocus : function ( input, previous ) {
      ...
      /*
      setTimeout( function() {
        if ( ! target.is(':focus') ) helper.hideCalendar( true );
        opt.controller.each( function(i,dom){
          var n = $(dom);
          if ( !n.is(':focus') ) n.removeClass('on_focus');
        });
      }, 50 );
      */
    },
    ...
    _click : function ( e ) {
      ...
      e.preventDefault();
      if ( current.hasClass('disabled') ||  current.hasClass('sb1_disabled') ) return;
    ...
    _updateInterval : function( stamp, where, toggler ) {
      ...
      if ( interval.length == 1 ) {
        if      ( interval[0] < opt.min_time ) interval[0] = opt.min_time;
        else if ( interval[0] > opt.max_time ) interval[0] = opt.max_time;

        opt.active[ interval[0] ] = true;
      }
      else if ( interval[0] === null && interval[1] ) {
        if ( interval[1] > opt.max_time ) interval[1] = opt.max_time;
    ...
    _getCalendarTable : function( date ) {
      ...
      var row = [], count = 0, day = opt.a_day, now = opt.now;
      var current = opt.now_time || 0, i = 0, j = 0;
      var active = opt.active || {}, min = opt.min_time || -1, max = opt.max_time || -1;
