...
function startup() {
  ...
  clickOnBlueLineToggleExpanderBtn( null, (readCookie(ATTR['cookie']+'_BL') ? true :false) );

  //$( document ).on('click', clickEvent).on('keyup', keyupEvent );
  $( document ).on('click',function(e){
    if ( ATTR['clickTimer'] ) clearTimeout(ATTR['clickTimer']);
    ATTR['clickTimer'] = setTimeout( function() { clickEvent(e); }, 30 );
  }).on('keyup', keyupEvent );
  ...
  $('.sb1_form_row', section).each(function(i,dom) { 
    ...
    node.find('.sb1_dropdown_menu').each( function (i,d) { 
      ...
      menu.find('>input').each(function(i,input) {
        var n = $(input), id = n.attr('id');
        if ( id && opts[id] ) n.attr('value', opts[id]);
      });
      ...
      if ( menu.hasClass('country_list') ) {
        ...
        menu.SB1dropdownMenu({
          ...
          'list' : ACCOUNT,
          'selected' : menu.find('input').val() || 'all',
...
function clickEvent( e ) {
  ...
  var order = [
    {'type':'class','what':'login_btn', 'grand': true,                 'handler': clickOnLoginBtn                },
    {'type':'class','what':'hamburger-wrapper', 'grand': true,         'handler': clickOnHamburgerWrapper        },
    {'type':'class','what':'close_login_wrapper',                      'handler': clickOnCloseLoginWrapper       },
    {'type':'class','what':'poster_option_btn', 'grand': true,         'handler': clickOnPosterOptionBtn         },
    ...
    {'type':'class','what':'blue_line_toggle_expander_btn',            'handler': clickOnBlueLineToggleExpanderBtn},
...
function clickOnBlueLineToggleExpanderBtn( data, collapse ) {
  var node = $('.blue_line_toggle_expander'), mode = 'collapse';
  var has  = typeof(collapse)==='boolean' ? ! collapse : node.hasClass(mode);
  if ( has ) {
    node.removeClass( mode );
    eraseCookie(ATTR['cookie']+'_BL');
  }
  else {
    node.addClass( mode );
    createCookie(ATTR['cookie']+'_BL', '1', 100);
  }
}
...
function changeMainContent( what, force, startup, category, style ) {
  ...
  if ( 
    what=='new_blue_line_icon' || what=='new_main_sidebar' || what=='new_top_banner' ||
    ...
    key  = what == 'new_top_banner' ? 'n_top_banner' :
      (what == 'new_blue_line_icon' ? 'n_blue_line' : (
