...
export const PersonWidget = ({data, onPress=()=>{}}) => {
  const actionList = [
    {'id': 'email', 'action': 'email-employee'},
    {'id': 'sms',   'action': 'sms-employee'  },
    {'id': 'call',  'action': 'call-employee' }
  ];

  return data ? <View style={styles.psersonWidget}>
    {actionList.map( (note, i)=> (
      <FormButton key={'person-widget-btn-'+i} label={note.id} type="plain"
        styleConfig={{}}
...
export default function EmployeeCard({
  ...
  return <View style={[styles.container, styleContainer]}>
    ...
    <View style={styles.actionContainer}>
      <Popup.Button onPress={(e)=>{onPress(e,'show-contact-detail', data)}}/>
    </View>
