...
/******************************************************************************
******************************************************************************/
export const convertTextToTimeStamp = (text) => {
  let t = (text ||'').replace(/^\s+/,'').replace(/\s+$/,'');
  if (! t) { return 0; }

  let r = /^(\d{4})[/\-.](0?[1-9]|1[012])[/\-.]([0][1-9]|[12][0-9]|3[01])(\w+(([0-1]\d)|(2[0-3])):([0-5]\d):([0-5]\d))?/;
  let m = t.match(r), s = m ? [m[1], m[2], m[3], m[5], m[8], (m[9]|| '0'), '0'] : null;
  if (! s) { return 0; }

  for (let i=0; i<s.length; i++) {
    s[i] = parseInt(((s[i] || '').replace(/^0/, '') || '0'), 10);
  }
  return (new Date(s[0],s[1]-1,s[2],s[3],s[4],s[5],s[6])).getTime();
};
...
/******************************************************************************
******************************************************************************/
export const getVcardString = (employee) => {
  if (! employee || ! employee.name) { return ''; }

  let start = 'BEGIN:VCARD', version= 'VERSION:3.0', end = 'END:VCARD';
  let note = [start,version], names = employee.name.trim().split(' ');
  let lName = names.pop(), fName = names.join(' ');
  note.push('N:' + lName + ';' + fName);

  if (employee.phone) {
    note.push('TEL;TYPE=CELL:' +employee.phone);
  }
  if (employee.email) {
    note.push('EMAIL;TYPE=PREF,INTERNET:' +employee.email);
  }
  ...
  if(Blob) {
    ...
    out = 'data:text/vcard;base64,' + this.btoa(text);
  }
...
/******************************************************************************
******************************************************************************/
export const getAlternativeName = (text) => {
  if (! text) { return text; }

  let list = [text], library = [
    { 'reg': /ndrz/gi, 'letter': 'ndr'},
    { 'reg': /(jci|dzi|cz)/gi, 'letter': 'ch'},
    { 'reg': /zh/gi, 'letter': 'z'},
    { 'reg': /sz/gi, 'letter': 's'},
    { 'reg': /ofz/gi, 'letter': 'az '},
    { 'reg': /w/gi, 'letter': 'v' },
    { 'reg': /aa/gi, 'letter': 'å' },
    { 'reg': /ee/gi, 'letter': 'i' },
    { 'reg': /ie/gi, 'letter': 'i' },
    ...
    { 'reg': /ka/gi, 'letter': 'ca' },
    { 'reg': /ca/gi, 'letter': 'ka' },
    { 'reg': /kob/gi, 'letter': 'cob'},
    { 'reg': /cob/gi, 'letter': 'kob'},
    { 'reg': /(ff|f)/gi, 'letter': 'ph' },
    { 'reg': /ph/gi, 'letter': 'f' },
    { 'reg': /k/gi, 'letter': 'ch' },
    { 'reg': /ch/gi, 'letter': 'k' },
    { 'reg': /x/gi, 'letter': 'ks' },
  ...
  library.forEach((d) => {
    ...
    for (i; i<length; i++) {
      if (! d.reg.test(list[i])) { continue; }
      list.push(list[i].replace(d.reg,d.letter));
    }
  });
...
/******************************************************************************
******************************************************************************/
export const removeTextAccent = (text) => {
  if (! text) { return text; }
  ...
  [
    ...
    text = text.replace(d.letters, d.base);
  });
  return text;
};
